...
export class RapierPhysicsManager {
  ...
  async initialize(
    ...
    // Apply variation jitter if needed (BEFORE storing initial positions!)
    const variation = config.variation ?? 0;
    if (variation > 0) {
      this.applyVariation(variation, config.seed ?? 42);
    }
    
    // Store initial positions for magnetic return (AFTER variation applied)
    this.bodies.forEach((body, meshId) => {
      const pos = body.translation();
      this.initialPositions.set(meshId, { x: pos.x, y: pos.y, z: pos.z });
    });
    console.log(`📍 Stored ${this.initialPositions.size} initial positions (sample keys: ${Array.from(this.initialPositions.keys()).slice(0, 3).join(', ')})`);
  ...
  startRecallBloomLoop(config: GravityEffectConfig): void {
    ...
    this.world.gravity = this.originalGravity;
    console.log("🌐 World gravity now:", this.originalGravity);
    
    // Wake all bodies for fall (sleeping is already enabled from creation)
    ...
    if (this.loopCfg && this.phase !== "idle") {
      ...
      if (this.phase === "fall") {
        ...
        if (this.phaseTimer >= halfDuration) {
          ...
          // 2e) Wake bodies for recall (they stay awake during recall phase)
          this.bodies.forEach(b => {
            b.wakeUp();
          });
        ...
        this.bodies.forEach((body, id) => {
          ...
          if (!goal) {
            console.warn(`⚠️ Missing goal for body ${id.slice(-8)}`);
            return;
          }
        ...
        if (Math.floor(this.phaseTimer * 2) % 1 === 0) {
          ...
          if (!it.done) {
            ...
            const p = b.translation();
            const v = b.linvel();
            console.log(`[recall] sample id=${id.slice(-8)} goal=${goal ? `(${goal.x.toFixed(2)},${goal.y.toFixed(2)},${goal.z.toFixed(2)})` : 'MISSING'} p=(${p.x.toFixed(2)},${p.y.toFixed(2)},${p.z.toFixed(2)}) v=(${v.x.toFixed(2)},${v.y.toFixed(2)},${v.z.toFixed(2)})`);
          }
        }
        ...
        if (this.postBloomTimer >= this.loopCfg!.postBloomDampenMs) {
          ...
          // Bodies can naturally sleep again after bloom
        }
      }
        ...
        if (this.loopHoldTimer >= this.loopCfg!.loopHoldMs) {
          ...
          // Bodies can naturally sleep for next fall cycle
  ...
  private createBloomBatch(batchSize: number): void {
    if (!this.RAPIER) {
      console.error('❌ RAPIER module not loaded in createBloomBatch');
      return;
    }
    if (!this.world) {
    ...
    console.log(`🌸 Creating batch of ${Math.min(batchSize, this.bloomQueue.length)} joints, RAPIER=${!!this.RAPIER}`);
    console.log('🔍 RAPIER keys:', Object.keys(this.RAPIER || {}));
    console.log('🔍 ImpulseJointDesc:', this.RAPIER?.ImpulseJointDesc);
    ...
    while (created < batchSize && this.bloomQueue.length > 0) {
      ...
      try {
        // Use JointData.fixed() API (4 params: anchor1, axis1, anchor2, axis2)
        const jointDesc = this.RAPIER.JointData.fixed(
          { x: 0, y: 0, z: 0 },           // anchor1
          { w: 1, x: 0, y: 0, z: 0 },     // axis1 (identity quaternion)
          { x: 0, y: 0, z: 0 },           // anchor2
          { w: 1, x: 0, y: 0, z: 0 }      // axis2 (identity quaternion)
        );
        const handle = this.world.createImpulseJoint(jointDesc, body1, body2, true);
        this.joints.push(handle);
        created++;
      } catch (err) {
        console.error(`❌ Failed to create joint between ${id1} and ${id2}:`, err);
      }
    }
    if (created > 0) {
  ...
  private handleStaggeredRelease(deltaTime: number, config: GravityEffectConfig): void {
    ...
    const rng = seededRandom((config.seed ?? 42) + numBatches);

...
export const GravityModal: React.FC<GravityModalProps> = ({
  ...
  const [config, setConfig] = useState<GravityEffectConfig>(initialConfig);
        ...
        <div style={{ 
          ...
          <div>
            ...
            <label style={{ display: 'flex', alignItems: 'center', gap: '0.5rem', cursor: 'pointer', marginBottom: '0.25rem' }}>
              <input
                type="checkbox"
                checked={config.environment.ground}
                onChange={(e) => setConfig({
                  ...config,
                  environment: { ...config.environment, ground: e.target.checked }
                })}
              />

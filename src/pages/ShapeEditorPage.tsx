  ...
  const onSave = async () => {
    ...
    try {
      ...
      const shapeFile: ShapeFile = {
        ...
        name: shapeName,
        cid: '', // Will be generated by Supabase
        cells: cells.map(c => [c.i, c.j, c.k]),
        meta: {
          lattice: 'fcc',
      ...
      const blob = new Blob([JSON.stringify(shapeFile, null, 2)], { type: 'application/json' });
      const file = new File([blob], `${shapeName}.fcc.json`, { type: 'application/json' });
      
      const shape = await uploadShape(file, shapeName, {
        cellCount: cells.length,
        lattice: 'fcc'
      });
      
      alert(`✅ Shape saved to cloud!\nID: ${shape.id}`);

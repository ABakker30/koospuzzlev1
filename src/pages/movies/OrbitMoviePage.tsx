// Orbit Movie Page - Standalone turntable effect viewer/recorder
...
export const OrbitMoviePage: React.FC = () => {
  ...
  const handleOrbitSave = (config: OrbitConfig) => {
    console.log('🎬 Orbit settings saved:', config);
    setShowOrbitModal(false);
  ...
  const handleShareSolution = () => {
    const shareUrl = `${window.location.origin}/movies/orbit/${solution?.id || id}`;
    ...
    <div style={{ 
      ...
      <div className="header" style={{
        ...
        <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
          <button
            className="pill"
            onClick={() => { closeAllModals(); setShowOrbitModal(true); }}
      ...
      >
        <div style={{ fontSize: '14px', lineHeight: '1.6', color: '#4b5563' }}>
          <p style={{ marginBottom: '16px' }}>
            <strong>Orbit Movie Page:</strong> Create and view 360° rotating animations of puzzle solutions.
          </p>
      ...
      <MovieWhatsNextModal
        ...
        movieTitle={movie?.title || solution?.puzzle_name || 'Orbit Movie'}
        isSaved={(() => {
      ...
      <ShareWelcomeModal
        ...
        movieTitle={movie?.title || 'Orbit Movie'}
      ...
      <ShareOptionsModal
        ...
        shareUrl={`${window.location.origin}/movies/orbit/${savedMovieId || movie?.id || id}?from=share`}
        movieTitle={movie?.title || solution?.puzzle_name || 'Orbit Movie'}

...
export const GravityMoviePage: React.FC = () => {
  ...
  useEffect(() => {
    if (user?.id) {
      console.log('🔄 [GravityMoviePage] Loading settings from DB for user:', user.id);
      settingsService.current.loadSettingsFromDB(user.id).then(dbSettings => {
        if (dbSettings) {
          console.log('✅ [GravityMoviePage] DB settings loaded');
  ...
  useEffect(() => {
    if (user?.id) {
      console.log('💾 [GravityMoviePage] Saving settings to DB');

// Explosion Movie Page - Standalone turntable effect viewer/recorder
...
import { DEFAULT_CONFIG } from '../../effects/explosion/presets';
...
export const ExplosionMoviePage: React.FC = () => {
  ...
  useEffect(() => {
    if (user?.id) {
      console.log('🔄 [ExplosionMoviePage] Loading settings from DB for user:', user.id);
      settingsService.current.loadSettingsFromDB(user.id).then(dbSettings => {
        if (dbSettings) {
          console.log('✅ [ExplosionMoviePage] DB settings loaded');
  ...
  useEffect(() => {
    if (user?.id) {
      console.log('💾 [ExplosionMoviePage] Saving settings to DB');
  ...
  const handleExplosionSave = (config: ExplosionConfig) => {
    console.log('🎬 Explosion settings saved:', config);
    setShowExplosionModal(false);
  ...
  const handleShareSolution = () => {
    const shareUrl = `${window.location.origin}/movies/explosion/${solution?.id || id}`;
    ...
    <div style={{ 
      ...
      <div className="header" style={{
        ...
        <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
          <button
            className="pill"
            onClick={() => { closeAllModals(); setShowExplosionModal(true); }}
      ...
      >
        <div style={{ fontSize: '14px', lineHeight: '1.6', color: '#4b5563' }}>
          <p style={{ marginBottom: '16px' }}>
            <strong>Explosion Movie Page:</strong> Create and view 360° rotating animations of puzzle solutions.
          </p>
      ...
      <MovieWhatsNextModal
        ...
        movieTitle={movie?.title || solution?.puzzle_name || 'Explosion Movie'}
        isSaved={(() => {
      ...
      <ShareWelcomeModal
        ...
        movieTitle={movie?.title || 'Explosion Movie'}
      ...
      <ShareOptionsModal
        ...
        shareUrl={`${window.location.origin}/movies/explosion/${savedMovieId || movie?.id || id}?from=share`}
        movieTitle={movie?.title || solution?.puzzle_name || 'Explosion Movie'}
...
export default ExplosionMoviePage;
